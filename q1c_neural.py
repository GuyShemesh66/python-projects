...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    h = sigmoid(np.dot(data ,W1) + b1)
    y_pred = softmax(np.dot(h ,W2) + b2)
    cost = - np.log(y_pred[np.where(y == 1)]).sum()
    ### END YOUR CODE

    ### YOUR CODE HERE: backward propagation
    gradW2 = np.dot(h.T, (y_pred - y))
    gradb2 = (y_pred - y).sum(axis=0).reshape(1, -1)
    gradW1 = np.dot(data.T , (np.dot(( y_pred - y) , W2.T) * sigmoid_grad(h)))
    gradb1 = (np.dot(( y_pred - y) , W2.T) * sigmoid_grad(h)).sum(axis=0).reshape(1, -1)
    ### END YOUR CODE
